...
def load_model() -> xgb.Booster:
    ...
    NOTE: loaded via the raw xgb.Booster API, not the XGBRegressor sklearn
    wrapper. XGBRegressor().load_model() throws
    "`_estimator_type` undefined. Please use appropriate mixin..." against
    this file — a known xgboost sklearn-wrapper metadata incompatibility
    (confirmed on xgboost 2.1.1 here), not something wrong with the model
    itself. Booster.load_model() reads the exact same JSON without needing
    that sklearn-specific metadata, and still exposes feature_names/
    feature_types directly off the file, so nothing else in this module
    changes behavior.
    """
...
def predict_next(station: str, history: pd.DataFrame) -> dict:
    ...
    dmatrix = xgb.DMatrix(features_df, feature_names=get_feature_names())
